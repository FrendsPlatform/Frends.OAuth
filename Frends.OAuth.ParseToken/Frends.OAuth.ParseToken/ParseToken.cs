...
public class OAuth
{
    ...
    /// <returns>Object { string Id, string Issuer, string Audiences, string SecurityKeyId, string SigningKeyId, DateTime ValidFrom, DateTime ValidTo, Dictionary&lt;string, dynamic&gt; Claims }</returns>
    public static async Task<ParseResult> ParseToken([PropertyTab] Input input, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
        ...
        var validateToken = handler.ValidateToken(input.GetToken(), validationParameters, out var validatedToken);
        var audiences = "";
