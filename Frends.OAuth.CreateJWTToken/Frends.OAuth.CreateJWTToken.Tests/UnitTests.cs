...
-----END RSA PRIVATE KEY-----"; //PK from community task
    ...
    public void CreateJwtTokenTestSymmetric()
    {
        ...
        Assert.IsTrue(result.Results.Count > 0 && result.Results != null);

        // JWT tokens always have 2 dot separators between parts.
        Assert.IsTrue(result.Results.Any(o => o.Token.Count(i => i.Equals('.')) == 2));
    }
